...
export function HRDashboard({ accessToken, profile, onNavigate }: { accessToken: string; profile: UserProfile; onNavigate: (page: string) => void }) {
  ...
  return <section className="hr-dashboard dashboard-workspace">
    ...
    <div className="dashboard-metric-grid">{metrics.map(([label, result, page]) => <button className="card metric dashboard-metric-card" key={String(label)} onClick={() => onNavigate(String(page))}><span>{label}</span><strong>{result}</strong><small>Open module</small></button>)}</div>
    ...
    <div className="dashboard-content-grid">
      <article className="card panel"><div className="panel-head"><div><h2>HR quick actions</h2><p className="muted">Common workforce operations</p></div></div><div className="quick dashboard-quick-grid">{[["Add employee", "Employee Management"], ["Start onboarding", "Onboarding"], ["Start offboarding", "Offboarding"], ["Review leave", "Leave Management"], ["Review expenses", "Expense Management"], ["Review assets", "Asset Management"], ["Create vacancy", "Recruitment"], ["Generate HR letter", "Documents & Templates"], ["Open HR tickets", "HR Help Desk"]].map(([label, page]) => <button key={label} onClick={() => onNavigate(page)}><span>↗</span>{label}</button>)}</div></article>
