...
function parseShiftMinutes(shift?: string) {
  const [h, m] = String(shift ?? "08:00")
    .split(":")
    .map(Number);
  return (h || 0) * 60 + (m || 0);
}
...
export function LiveAttendancePage({
    ...
    try {
      const [emp, att, pol] = await Promise.all([
        listNamedRows(
          accessToken,
          "employees",
          "id,first_name,last_name,department_name,position_title,employment_status",
          "first_name",
        ),
        listRowsWhere(
          accessToken,
      ...
      setEmployees(
        emp.filter(
          (row) =>
            !["terminated", "offboarded"].includes(
  ...
  // loadAttendancePolicy is an RPC and RPC completion emits the global mutation event. Listening
  // to that same event here caused a self-triggering reload loop and a continuously blinking
  // Refresh button. Attendance changes remain available through the explicit refresh control.

  const lateThreshold =
    parseShiftMinutes(policy.shift_start) + Number(policy.grace_minutes ?? 0);
  ...
  const rows: Row[] = useMemo(
    () =>
      employees.map((employee) => {
        ...
        const status: Status = !clockIn
          ? "not_clocked_in"
          : minutesSinceMidnight(new Date(String(clockIn))) > lateThreshold
            ? "late"
            : "present";
        return {
  ...
  const notClockedInCount = rows.filter(
    (row) => row.status === "not_clocked_in",
  ).length;
    ...
    <section>
      <header className="page-header">
        <div>
          ...
          <p className="muted">
            Who has clocked in and out, for any day. Late is anyone clocking in
            after {String(policy.shift_start ?? "08:00").slice(0, 5)} plus the{" "}
            {policy.grace_minutes ?? 0} minute grace period set in the
            attendance policy.
          </p>
        </div>
      ...
      <article className="card data-panel">
              ...
              {visible.map((row) => (
                <tr key={row.employeeId}>
                  <td>
                    ...
                    <small className="table-subline">{row.position}</small>
                  </td>
                  ...
                  <td>
                    <span
                      className={`status-pill ${row.status === "present" ? "active" : row.status === "late" ? "pending" : "archived"}`}
                    >
                      {row.status === "present"
                        ? "On time"
                        : row.status === "late"
        ...
        {!visible.length && !loading && (
          <div className="empty-state">
            ...
            <p>Change the filters or pick another date.</p>
          </div>
        )}
      </article>
    </section>
