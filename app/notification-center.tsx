...
import { NotificationDetail } from "./notifications-page";
...
export function NotificationCenter({accessToken,profile,onNavigate}:{accessToken:string;profile:UserProfile;onNavigate:(target:string)=>void}) {
  const [open,setOpen]=useState(false),[items,setItems]=useState<DataRow[]>([]),[filter,setFilter]=useState<Filter>("all"),[sort,setSort]=useState<Sort>("newest"),[query,setQuery]=useState(""),[busy,setBusy]=useState(""),[selected,setSelected]=useState<DataRow|null>(null);
