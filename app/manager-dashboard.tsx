...
export function ManagerDashboard({ accessToken, profile, onNavigate }: { accessToken: string; profile: UserProfile; onNavigate: (page: string) => void }) {
  ...
  return <section className="dashboard-workspace">
    ...
    <div className="dashboard-metric-grid">{cards.map(([label, result, page]) => <button className="card metric dashboard-metric-card" key={label} onClick={() => onNavigate(page)}><span>{label}</span><strong>{result}</strong><small>Open module</small></button>)}</div>
    ...
    <div className="quick dashboard-quick-grid manager-quick">{[["Leave Approvals", "Approve leave"], ["Team Attendance", "Review attendance"], ["Tasks", "Assign task"], ["Team Performance", "Start performance review"], ["One to One Meetings", "Schedule one to one"], ["Recruitment & Onboarding", "Submit recruitment request"], ["Expense Approvals", "Approve expense"], ["Learning & Development", "Assign training"], ["Team Communication", "Send team message"], ["Meetings & Calendar", "View team calendar"], ["Documents", "Request employee document"], ["Employee Requests", "Review employee requests"]].map(([page, label]) => <button key={page} onClick={() => onNavigate(page)}><span>→</span>{label}</button>)}</div>
