...
export function EmployeeHome({
  ...
  const moreEntries = canOpenLibrary
    ? [...moreItems, ["library", "Book Library"] as [Tab, string]]
    : moreItems;
  ...
  useEffect(() => {
    setTab(sectionTab[activeSection] ?? "dashboard");
    setMoreOpen(false);
  }, [activeSection]);
  useEffect(() => {
    setNotice("");
  ...
  const load = useCallback(async () => {
    ...
    const own = async (table: string, key = "employee_id", limit = 500) => {
      try {
        ...
        issues.push(
          `${table}: ${cause instanceof Error ? cause.message : "query failed"}`,
        );
        return [];
      }
    };
    ...
    const [
      ...
      listRowsWhere(accessToken, "employees", { id: employeeId }, "*", 1).catch(
        (cause) => {
          issues.push(
            `employees: ${cause instanceof Error ? cause.message : "query failed"}`,
          );
          return [];
        },
      ),
      ...
      own("asset_requests"),
      own("employee_benefits"),
      organisation("job_openings", 100),
      own("internal_job_applications"),
      organisation("announcements", 100),
      own("messages", "recipient_employee_id"),
      organisation("meetings", 100),
      organisation("company_holidays", 100),
      own("support_tickets"),
      listRowsWhere(
        accessToken,
        "notifications",
        ...
        issues.push(
          `notifications: ${cause instanceof Error ? cause.message : "query failed"}`,
        );
        return [];
      }),
      own("employee_change_requests"),
      own("employee_awards"),
      own("employment_history"),
      listRowsWhere(
        accessToken,
        "user_preferences",
        ...
        issues.push(
          `user_preferences: ${cause instanceof Error ? cause.message : "query failed"}`,
        );
        return [];
      }),
      own("transfer_requests"),
    ]);
    ...
    if (issues.length)
      setError(
        `Some employee modules could not be refreshed. ${issues.slice(0, 3).join(" · ")}${issues.length > 3 ? ` · ${issues.length - 3} more` : ""}`,
      );
  ...
  const todayRecord = data.attendance.find(
    (row) => String(row.attendance_date) === today,
  );
  ...
  const liveSeconds = clockedIn
    ? Math.max(
        0,
        Math.floor(
          (now - new Date(String(todayRecord?.clock_in)).getTime()) / 1000,
        ),
      )
    : 0;
      ...
      data.attendance.reduce(
        (sum, row) =>
          row.clock_in && row.clock_out
            ? sum +
              (new Date(String(row.clock_out)).getTime() -
                new Date(String(row.clock_in)).getTime()) /
                3600000
            : sum,
        0,
      ),
  ...
  const attendanceRate = data.attendance.length
    ? Math.round(
        (data.attendance.filter((row) =>
          ["present", "late", "remote", "travel"].includes(String(row.status)),
        ).length /
          data.attendance.length) *
          100,
      )
    : 0;
  const overtime = data.attendance.reduce(
    (sum, row) =>
      sum +
  ...
  const pending =
    ...
    data.assetRequests.filter((row) =>
      ["pending", "manager_review", "hr_review", "approved"].includes(
        String(row.status),
      ),
    ).length +
    data.requests.filter((row) =>
      ["draft", "pending", "submitted"].includes(String(row.status)),
    ).length;
  const openTasks = data.tasks.filter(
    (row) => !["completed", "cancelled", "closed"].includes(String(row.status)),
  ).length;
  const performanceScore = Number(
    data.performance[0]?.rating ?? data.performance[0]?.score ?? 0,
  );
  ...
  async function attendanceAction(
    ...
    if (!profile.employee_id)
      return setError("HR must link your login to an employee record first.");
    setBusy(action);
    setError("");
    setNotice("");
    try {
      ...
      setError(
        cause instanceof Error ? cause.message : "Attendance action failed.",
      );
    } finally {
      setBusy("");
    }
  ...
  function askAi(event: FormEvent) {
    ...
    let answer =
      "I could not find a direct answer. Submit an HR support ticket for a verified response.";
    if (q.includes("leave"))
      answer = `You currently have ${leaveBalance} leave days recorded and ${pending} pending self-service request(s). Use Leave to apply and track approval.`;
    else if (q.includes("pay") || q.includes("salary"))
      answer = `Your payroll workspace contains ${data.payroll.length} authorised payslip record(s). Your profile salary is ${money(employee?.monthly_salary ?? employee?.basic_salary)} monthly where configured.`;
    else if (
      q.includes("attendance") ||
      q.includes("late") ||
      q.includes("overtime")
    )
      answer = `Your attendance rate is ${attendanceRate}% with ${workedHours.toFixed(1)} recorded hours and ${overtime.toFixed(1)} overtime hours.`;
    else if (q.includes("training") || q.includes("course"))
      answer = `You have ${data.learning.length} assigned training record(s). Open Learning for progress and due dates.`;
    else if (q.includes("career") || q.includes("promotion"))
      answer = `There are ${data.jobs.filter((row) => ["open", "published"].includes(String(row.status))).length} open internal vacancies.`;
    else if (q.includes("policy") || q.includes("document"))
      answer = `You have ${data.documents.length} employee document(s) available.`;
    setAiAnswer(answer);
  }

  return (
    <section className="employee-portal-v2">
      {error && (
      ...
      {!profile.employee_id && (
        <article className="card data-panel">
          <h2>Employee record not linked</h2>
          <p>
            Ask HR to link this login to an employee record. Personal modules
            remain protected until linking is complete.
          </p>
        </article>
      )}

      {activeSection === "Home" && (
        <div className="home-hero-row employee-home-hero">
          <header className="page-header">
            <div>
              <span className="eyebrow">Employee self service</span>
              <h1>Welcome, {firstName}</h1>
              <p className="muted">
                {employee?.position_title || profile.job_title || "Employee"} ·{" "}
                {employee?.department_name || "Your department"} ·{" "}
          ...
          <article className="card employee-clock quick-attendance-card compact">
                ...
                <p className="muted">
                  {onBreak
                    ...
                    : clockedIn
                      ...
                      : "Clock in to begin today's attendance."}
                </p>
              </div>
              <strong className="attendance-live-time">
                {clockedIn ? liveTime : "00:00"}
              </strong>
            </div>
            <div className="row-actions attendance-quick-actions">
              <button
                className="primary"
                disabled={clockedIn || Boolean(busy)}
                onClick={() => void attendanceAction("clock_in")}
              >
                Clock in
              </button>
              <button
                disabled={!clockedIn || onBreak || Boolean(busy)}
                onClick={() => void attendanceAction("break_in")}
              >
                Start break
              </button>
              <button
                disabled={!onBreak || Boolean(busy)}
                onClick={() => void attendanceAction("break_out")}
              >
                Resume
              </button>
              <button
                className="danger"
                disabled={!clockedIn || Boolean(busy)}
                onClick={() => void attendanceAction("clock_out")}
              >
                Clock out
              </button>
            </div>
      ...
      {activeSection === "My Info" && (
        <div className="employee-info-tabs-row">
          <nav
            ...
            aria-label="My Info sections"
          >
            {infoTabs.map(([id, label]) => (
              <button
                type="button"
                key={id}
            ...
            <button
              type="button"
              className={moreEntries.some(([id]) => id === tab) ? "active" : ""}
          ...
          {moreOpen && (
            <div className="account-menu more-menu employee-info-more-menu">
              {moreEntries.map(([id, label]) => (
                <button
                  type="button"
                  key={id}
                  onClick={() => {
      ...
      {tab === "tasks" && (
        <TaskPage rows={data.tasks} accessToken={accessToken} onReload={load} />
      )}
      {tab === "expenses" && (
        <RecordPage
      ...
      {tab === "requests" && (
        <RecordPage
          title="My requests"
          subtitle="Track all employee self-service requests in one place"
          rows={[
            ...data.leave.map((row) => ({
              ...row,
              request_type: "Leave",
              request_title: row.leave_type,
              request_update: row.hr_comment ?? row.manager_comment,
            })),
            ...data.expenses.map((row) => ({
              ...row,
              request_type: "Expense",
              request_title: row.expense_type ?? row.category,
              request_update: row.hr_comment ?? row.manager_comment,
            })),
            ...data.assetRequests.map((row) => ({
              ...row,
              request_type: "Asset",
              request_title: row.asset_type ?? row.category,
              request_update: row.hr_comment ?? row.manager_comment,
            })),
            ...data.transfers.map((row) => ({
              ...row,
              request_type: "Transfer",
              request_title: row.requested_department ?? row.requested_branch,
              request_update: row.hr_comment ?? row.manager_comment,
            })),
            ...data.requests.map((row) => ({
              ...row,
              request_type: "Profile change",
              request_title: row.field_name,
              request_update: row.review_note ?? row.manager_comment,
            })),
          ]
            .map((row) => row as DataRow)
            .sort((a, b) =>
      ...
      {tab === "assets" && (
        <>
          <RecordPage
            ...
            subtitle="Equipment assigned to your custody and return history"
            rows={data.assets}
            columns={[
              ["asset_code", "Asset ID"],
              ["category", "Asset"],
          ...
          <RecordPage
            ...
            subtitle="Requests awaiting HR or administrator action"
            rows={data.assetRequests}
            columns={[
              ["asset_type", "Asset needed"],
              ["reason", "Reason"],
              ["priority", "Priority"],
              ["created_at", "Submitted"],
              ["status", "Status"],
      ...
      {tab === "notifications" && (
        <NotificationsPage
          ...
          accessToken={accessToken}
          onPreferences={onNotificationSettings}
          onReload={load}
        />
      )}
      {tab === "analytics" && (
    ...
    .map(
      ([label, result]) =>
        [
          label,
          String(result),
          Number(String(result).replace(/[^0-9.-]/g, "")) || 0,
  ...
  return (
    <>
      ...
      <article className="card dashboard-insights employee-home-summary">
        <div className="panel-head">
          <div>
            <h2>My quick summary</h2>
            <p className="muted">
              Live indicators from your employee workspace
            </p>
          </div>
        </div>
        <div className="insight-bars">
          {summaryBars.map(([label, raw, value]) => (
            <div className="insight-row" key={label}>
              ...
              <div className="insight-track">
                <span
                  style={{
                    width: `${Math.min(100, (value / summaryMax) * 100)}%`,
                  }}
                />
              </div>
            </div>
      ...
      <div className="employee-dashboard-grid">
        <article className="card employee-profile-card">
          ...
          <div className="profile-summary">
            <div className="profile-photo">
              {String(employee?.first_name ?? "E").slice(0, 1)}
              {String(employee?.last_name ?? "").slice(0, 1)}
            </div>
            <div>
              <h3>
              ...
              <p>{String(employee?.position_title ?? "Employee")}</p>
              <span>
                {String(employee?.employee_number ?? "Staff number pending")}
              </span>
            </div>
          </div>
          <button className="secondary wide-button" onClick={onProfile}>
            View full profile
          </button>
        </article>
        <article className="card span-two">
          <div className="panel-head">
            <div>
              <h2>Quick actions</h2>
              <p className="muted">Common self service tasks</p>
            </div>
          </div>
          <QuickActionsGrid
            items={[
        ...
        <ListCard
          ...
          rows={data.announcements
            ...
            .slice(0, 5)
            .map((row) => ({
              icon: "announcement" as IconName,
              iconColor: "var(--viz-purple-strong)",
              title: String(row.title ?? "Announcement"),
              subtitle: String(row.body ?? "").slice(0, 90),
            }))}
          emptyLabel="No announcements have been published yet."
        />
        <ListCard
          title="Recent Notifications"
          rows={data.notifications.slice(0, 5).map((row) => ({
            icon: "bell" as IconName,
            iconColor: "var(--viz-blue)",
            title: String(row.title ?? "Notification"),
            subtitle: String(row.created_at ?? ""),
          }))}
          emptyLabel="No notifications yet."
        />
        <ListCard
          title="Upcoming Events"
          rows={upcomingEvents.map((row) => ({
            icon: "calendar" as IconName,
            iconColor: "var(--viz-orange-strong)",
            title: String(row.title ?? "Event"),
            trailing: {
              type: "pill" as const,
              label: new Date(String(row.starts_at)).toLocaleDateString(
                "en-GB",
        ...
        <ListCard
          ...
          rows={data.learning
            ...
            .map((row) => ({
              ...
              iconColor: "var(--viz-slate)",
              title: String(row.course_name ?? "Training"),
              subtitle: `Due ${row.due_date ?? "—"}`,
            }))}
        ...
        <ListCard
          ...
          rows={data.benefits
            ...
            .slice(0, 5)
            .map((row) => ({
              icon: "benefit" as IconName,
              iconColor: "var(--viz-red-strong)",
              title: String(row.benefit_name ?? "Benefit"),
              subtitle: `Since ${row.start_date ?? "—"}`,
            }))}
          emptyLabel="No active benefits on file."
        />
        <ListCard
          title="My Requests"
          action={{ label: "View All", onClick: () => onTab("requests") }}
          rows={[
            ...data.leave.map((row) => ({
              ...row,
              request_label: row.leave_type ?? "Leave request",
            })),
            ...data.expenses.map((row) => ({
              ...row,
              request_label:
                row.expense_type ?? row.category ?? "Expense claim",
            })),
            ...data.assetRequests.map((row) => ({
              ...row,
              request_label: row.asset_type ?? row.category ?? "Asset request",
            })),
            ...data.transfers.map((row) => ({
              ...row,
              request_label: row.requested_department ?? "Transfer request",
            })),
          ]
            .map((row) => row as DataRow)
            .sort((a, b) =>
            ...
            .slice(0, 5)
            .map((row) => ({
              icon: "profile-request" as IconName,
              iconColor: "var(--sas-cyan-deep)",
              title: String(row.request_label),
              subtitle: String(row.status ?? "pending").replaceAll("_", " "),
            }))}
          emptyLabel="No requests submitted yet."
        />
        <article className="card span-two ai-teaser">
          <div>
            <span className="eyebrow">AI HR Assistant</span>
            <h2>Get answers from your employee workspace</h2>
            <p>
              Ask about leave, attendance, payroll, policies, documents,
...
function ProfileHero({
  ...
  useEffect(() => {
    let cancelled = false;
    (photoPath
      ? createSignedStorageUrl(accessToken, "employee-media", String(photoPath))
      : Promise.resolve("")
    )
      .then((url) => {
  ...
  return (
    <div className="profile-banner">
      <div className="profile-banner-avatar">
        {photoUrl ? (
          <img src={photoUrl} alt={`${fullName} profile`} />
        ) : (
          initials
        )}
      </div>
      <div className="profile-banner-info">
        ...
        <h1>{fullName}</h1>
        <p>
          {String(employee?.position_title ?? profile.job_title ?? "Employee")}
        ...
        <span>
          {employee?.employee_number
            ? `Staff #${String(employee.employee_number)}`
            : "Staff number pending"}
          {employee?.branch ? ` · ${String(employee.branch)}` : ""}
        </span>
      </div>
      <button
        type="button"
        className="secondary profile-banner-action"
        onClick={onRequest}
      >
        Request a change
      </button>
  ...
  const sections: [string, [string, unknown][]][] = [
    ...
    [
      ...
      [
        ...
        ["Reporting manager", employee?.manager_name],
    ...
    [
      "Bank, tax and pension",
      [
        ["Bank", employee?.bank_name],
        ["Account name", employee?.bank_account_name],
        ["Account number", mask(employee?.bank_account_number)],
        ["Tax number", mask(employee?.tax_number)],
        ["SSNIT / pension", mask(employee?.ssnit_number)],
        ["Pension provider", employee?.pension_provider],
      ],
    ],
  ];
  return (
        ...
        <div className="profile-layout-main">
          ...
          <div className="employee-profile-grid">
            {sections.map(([title, rows]) => (
              <DetailCard key={title} title={title} rows={rows} />
            ))}
          </div>
          <RecordPage
            title="Employment history"
            subtitle="Role, department, promotion and transfer history"
            rows={history}
            columns={[
...
function BasicInformationCard({
  ...
  const dob = employee?.date_of_birth
    ? new Date(String(employee.date_of_birth))
    : null;
  const age =
    dob && !Number.isNaN(dob.getTime())
      ? Math.floor((new Date().getTime() - dob.getTime()) / 31557600000)
      : null;
  const fields: [string, string][] = [
    ...
    [
      ...
      employee?.date_of_birth
        ? `${value(employee.date_of_birth)}${age !== null ? ` (Age ${age})` : ""}`
        : "—",
    ],
  ...
  return (
    <article className="card data-panel basic-information-card">
      <div className="panel-head">
        <h2>
          <MenuIcon name="badge" />
          Basic Information
      ...
      <div className="basic-info-grid">
        {fields.map(([label, field]) => (
          <label key={label}>
            <span>{label}</span>
            <div className="field-box">{field}</div>
          </label>
        ))}
      </div>
    </article>
...
function VitalsPanel({ employee }: { employee: DataRow | null }) {
  ...
  const rows = (
    [
      ...
      [
        "Local time",
        now.toLocaleTimeString("en-GB", { hour: "numeric", minute: "2-digit" }),
        "attendance",
        undefined,
      ],
    ...
    ] as [string, unknown, VitalsIcon, string | undefined][]
  ).filter(([, entry]) => entry);
  const hireDate = employee?.start_date
    ? new Date(String(employee.start_date))
    : null;
  const tenure =
    hireDate && !Number.isNaN(hireDate.getTime())
      ? tenureLabel(hireDate)
      : null;
  return (
    <article className="card data-panel vitals-panel">
      <h2>Vitals</h2>
      <div className="vitals-list">
        {rows.map(([label, entry, icon, subtitle]) => (
          <div key={label}>
            <span className="vitals-icon" title={label}>
              <MenuIcon name={icon} />
            </span>
            <div>
        ...
        {!rows.length && <p className="muted">No vitals on file yet.</p>}
      </div>
      {hireDate && !Number.isNaN(hireDate.getTime()) && (
        <div className="vitals-hire">
          <span className="vitals-icon" title="Hire date">
            <MenuIcon name="calendar" />
          </span>
          <div>
            <strong>{value(employee?.start_date)}</strong>
            {tenure && <small>{tenure}</small>}
          </div>
        </div>
      )}
    </article>
...
function tenureLabel(start: Date) {
  const months = Math.max(
    0,
    Math.floor((Date.now() - start.getTime()) / 2629800000),
  );
...
function AttendancePage({
  ...
  async function submitProof(request: DataRow) {
    const proof = window.prompt(
      "Describe the work completed during overtime and include any supporting reference:",
      String(request.employee_proof ?? ""),
    );
    if (!proof?.trim()) return;
    setProofBusy(String(request.id));
    setProofError("");
    try {
      await callRpc(accessToken, "submit_my_overtime_proof", {
  ...
  return (
    <>
      <header className="page-header">
        <div>
          <h1>
            <MenuIcon name={moduleIcon("Attendance")} />
            Attendance
          </h1>
          <p className="muted">
            Clocking, breaks, calendar, history and analytics.
          </p>
        </div>
      </header>
      <div className="employee-quick-cards">
        <article>
          <span>Live status</span>
          <strong>
            {onBreak ? "On break" : clockedIn ? "Working" : "Not clocked in"}
          </strong>
        </article>
        <article>
          <span>Live time</span>
          <strong>{clockedIn ? liveTime : "00:00:00"}</strong>
        </article>
        <article>
        ...
        <article>
          <span>Overtime</span>
          <strong>{overtime.toFixed(1)}</strong>
        </article>
      </div>
      <div className="employee-actions attendance-actions">
        <button
          className="primary"
          disabled={clockedIn || Boolean(busy)}
      ...
      {overtimeRequests.length > 0 && (
        <article className="card data-panel">
          <div className="section-heading">
            <div>
              <span className="eyebrow">My overtime</span>
              <h2>Overtime approval status</h2>
            </div>
          </div>
              ...
              <tbody>
                {overtimeRequests.map((request) => (
                  <tr key={String(request.id)}>
                    ...
                    <td>{String(request.reviewer_comment ?? "—")}</td>
                    <td>
                      {String(request.employee_proof ?? "—")}
                      {request.status === "questions" && (
                        <button
                          className="secondary"
                          disabled={proofBusy === String(request.id)}
                          onClick={() => void submitProof(request)}
                        >
                          Send proof
                        </button>
                      )}
      ...
      <RecordPage
        ...
        subtitle="Late arrivals, early departures, overtime and daily attendance"
        rows={rows}
        columns={[
          ...
          ["status", "Status"],
          ["late_minutes", "Late"],
          ["early_departure_minutes", "Early departure"],
      ...
      <div className="attendance-calendar">
        {Array.from({ length: days }, (_, index) => index + 1).map((day) => {
          const date = `${year}-${String(month + 1).padStart(2, "0")}-${String(day).padStart(2, "0")}`,
            record = rows.find((row) => String(row.attendance_date) === date);
          return (
            <div key={day} className={record ? `status-${record.status}` : ""}>
              <strong>{day}</strong>
              <span>
                {record ? String(record.status).replaceAll("_", " ") : "—"}
              </span>
            </div>
          );
        })}
  ...
  return (
    <>
      <header className="page-header">
        <div>
          <h1>
            <MenuIcon name={moduleIcon("Leave")} />
            Leave Management
          </h1>
          <p className="muted">
            Apply, monitor balances and review leave history.
          </p>
        </div>
      ...
      <RecordPage
        ...
        columns={[
          ...
          ["status", "Status"],
          ["reason", "Reason"],
          ["manager_comment", "Manager update"],
          ["hr_comment", "HR update"],
        ]}
        downloadable
        accessToken={accessToken}
      ...
      <RecordPage
        ...
        subtitle="Organisation public holidays"
        rows={holidays}
        columns={[
          ["holiday_date", "Date"],
          ["name", "Holiday"],
...
function PayrollPage({
  ...
  const [payslipTemplate, setPayslipTemplate] = useState<DataRow | null>(null);
  useEffect(() => {
    let cancelled = false;
    listRowsWhere(
      accessToken,
      ...
      .then((rows) => {
        if (!cancelled) setPayslipTemplate(rows[0] ?? null);
      })
      .catch(() => {
        if (!cancelled) setPayslipTemplate(null);
      });
    return () => {
      cancelled = true;
    };
  ...
  function download() {
    if (!current) return;
    if (payslipTemplate && employee) {
      const content = renderTemplateContent(
        String(payslipTemplate.content ?? ""),
        employee,
        current,
      );
      printTemplateDocument(String(payslipTemplate.name ?? "Payslip"), content);
    } else {
      printRows(
        "Payslip",
        [current],
    ...
    <>
      <header className="page-header">
        <div>
          ...
          <p className="muted">
            Current payslip, payroll history and authorised salary information.
          </p>
        </div>
      </header>
      {!current &&
        Number(employee?.monthly_salary ?? employee?.basic_salary ?? 0) > 0 && (
          <article className="card data-panel">
            <h2>Salary configured</h2>
            <p>
              Your monthly basic salary is{" "}
              {money(employee?.monthly_salary ?? employee?.basic_salary)}. A
              payslip will appear after HR publishes the payroll period.
            </p>
          </article>
        )}
      {current && (
        <article className="card payslip-card">
          <div>
            <span>Current payslip</span>
            <h2>{String(current.pay_period)}</h2>
            <p>Net pay</p>
            <strong>{money(current.net_pay)}</strong>
          </div>
          <div className="payslip-breakdown">
            <span>
              Basic salary <b>{money(current.basic_salary)}</b>
            </span>
            <span>
              Bonuses <b>{money(current.bonuses)}</b>
            </span>
            <span>
              Allowances <b>{money(current.allowances)}</b>
            </span>
            <span>
              PAYE <b>{money(current.paye_tax)}</b>
            </span>
            <span>
              Employee SSNIT <b>{money(current.employee_ssnit)}</b>
            </span>
            <span>
              Tier 2 <b>{money(current.tier_two)}</b>
            </span>
            <span>
              Tier 3 <b>{money(current.tier_three)}</b>
            </span>
            <span>
              Other deductions <b>{money(current.other_deductions)}</b>
            </span>
          </div>
      ...
      <RecordPage
        ...
        subtitle="Previous published payslips and payroll releases"
        rows={rows}
        columns={[
          ["pay_period", "Period"],
          ["basic_salary", "Basic salary"],
...
function DocumentsPage({ rows }: { rows: DataRow[] }) {
  return (
    <RecordPage
      title="Documents"
      subtitle="Contracts, appointment and promotion letters, HR letters, warnings, handbooks, policies, tax forms and certificates"
      rows={rows}
      columns={[
    ...
    <>
      <div className="employee-quick-cards">
        <article>
          ...
          <strong>
            {String(reviews[0]?.rating ?? reviews[0]?.score ?? "—")}
          </strong>
        </article>
        <article>
          <span>Active goals</span>
          <strong>
            {goals.filter((row) => row.status !== "completed").length}
          </strong>
        </article>
        <article>
      ...
      <RecordPage
        ...
        subtitle="Self assessment, manager feedback and review history"
        rows={reviews}
        columns={[
          ["review_type", "Review"],
      ...
      <RecordPage
        ...
        subtitle="Recognition, awards and career development"
        rows={awards}
        columns={[
          ["title", "Award"],
          ["award_date", "Date"],
...
function LearningPage({
  ...
  async function updateJourney(row: DataRow, status: string) {
    const progress =
      status === "completed"
        ? 100
        : Number(
            window.prompt(
              ...
              String(row.progress ?? 0),
            ) ??
              row.progress ??
              0,
          );
    const note =
      window.prompt("Optional note or item needing attention:", "") ?? "";
    setBusy(String(row.id));
    setError("");
    try {
      await callRpc(accessToken, "update_my_onboarding_status", {
      ...
      setError(
        cause instanceof Error
          ? cause.message
          : "Onboarding status could not be updated.",
      );
    } finally {
      setBusy("");
    }
  ...
  return (
    <>
        ...
        <article className="card data-panel">
          <div className="panel-head">
            <div>
              <h2>My onboarding</h2>
              <p className="muted">
                Confirm your progress or flag an item that needs attention.
              </p>
            </div>
          </div>
            ...
            {onboarding.map((row) => (
              <div key={String(row.id)}>
                <span>
                  ...
                  <small>
                    {Number(row.progress ?? 0)}% complete · due{" "}
                ...
                <div className="row-actions">
                  <button
                    ...
                    onClick={() => void updateJourney(row, "in_progress")}
                  >
                    In progress
                  </button>
                  <button
                    disabled={busy === String(row.id)}
                    onClick={() => void updateJourney(row, "needs_attention")}
                  >
                    Needs attention
                  </button>
                  <button
                    disabled={busy === String(row.id)}
                    onClick={() => void updateJourney(row, "completed")}
                  >
                    Mark complete
                  </button>
                </div>
      ...
      <RecordPage
        ...
        subtitle="Assigned courses, company training, certifications, skills and workshops"
        rows={rows}
        columns={[
          ["course_name", "Course"],
          ["training_type", "Type"],
...
function BenefitsPage({ rows }: { rows: DataRow[] }) {
  return (
    <RecordPage
      title="Benefits"
      subtitle="Health insurance, pension, medical benefits, staff loans, allowances and enrollment"
      rows={rows}
      columns={[
  ...
  return (
    <>
      <header className="page-header">
        <div>
          <h1>
            <MenuIcon name={moduleIcon("Recruitment")} />
            Internal Recruitment
          </h1>
          <p className="muted">
            Vacancies, promotion opportunities, transfer requests and
            applications.
          </p>
        </div>
      ...
      <div className="job-cards">
        {openJobs.map((job) => (
          <article
            ...
            tabIndex={0}
            onKeyDown={(event) => {
              if (event.key === "Enter") setViewing(job);
            }}
          >
            <span className="eyebrow">
              {String(job.employment_type ?? "Internal opportunity")}
            </span>
            <h2>{String(job.title)}</h2>
            <p>
              {String(job.location ?? "")} · closes {value(job.closing_date)}
            </p>
        ...
        {!openJobs.length && (
          <div className="empty-state">
            ...
            <p>Internal opportunities will appear here when published.</p>
          </div>
        )}
      </div>
      ...
      <RecordPage
        ...
        subtitle="Requested department, branch and manager decisions"
        rows={transfers}
        columns={[
      ...
      {viewing && (
        ...
        >
          <section className="modal" role="dialog" aria-modal="true">
            ...
            <span className="eyebrow">
              {String(viewing.employment_type ?? "Internal opportunity")}
            </span>
            <h2>{String(viewing.title)}</h2>
            <p className="muted">
              {String(viewing.location ?? "")} · closes{" "}
              {value(viewing.closing_date)}
            </p>
            <h3>Description</h3>
            <p style={{ whiteSpace: "pre-wrap" }}>
              {String(viewing.description ?? "No description provided.")}
            </p>
            {viewing.requirements ? (
              <>
                <h3>Requirements</h3>
                <p style={{ whiteSpace: "pre-wrap" }}>
                  {String(viewing.requirements)}
                </p>
              </>
            ) : null}
...
function ApplyDialog({
  ...
  async function submit(event: FormEvent) {
    event.preventDefault();
    if (!employeeId)
      return setError("Your login is not linked to an employee record.");
    setBusy(true);
    setError("");
    try {
      let resumePath = "";
      if (resume) {
        ...
        resumePath = `applications/${employeeId}/${job.id}.${extension}`;
      ...
      await createRow(accessToken, "internal_job_applications", {
  ...
  return (
    ...
    >
      <section className="modal record-modal" role="dialog" aria-modal="true">
        <button type="button" className="modal-close" onClick={onClose}>
          ×
        </button>
        <span className="eyebrow">Internal recruitment</span>
        <h2>Apply for {String(job.title)}</h2>
        <form className="record-form" onSubmit={submit}>
          <label className="wide" htmlFor="internal-application-cover-note">
            Cover note
            <textarea
              id="internal-application-cover-note"
              name="cover_note"
              value={coverNote}
              onChange={(event) => setCoverNote(event.target.value)}
              placeholder="Tell the hiring manager why you're a good fit..."
            />
          </label>
          <label className="wide" htmlFor="internal-application-resume">
            Attach CV / resume (optional)
            <input
              id="internal-application-resume"
              name="resume"
              type="file"
              accept=".pdf,.doc,.docx"
...
function CommunicationPage({
  ...
  return (
    <>
      <RecordPage
        title="Company announcements"
        subtitle="Published company communication"
        rows={announcements.filter((row) => row.status === "published")}
        columns={[
          ["title", "Announcement"],
          ["body", "Message"],
      ...
      />
      <RecordPage
        title="HR and manager messages"
        subtitle="Direct, HR and team communication"
        rows={messages}
        columns={[
          ["sender_name", "From"],
          ["subject", "Subject"],
      ...
      />
      <RecordPage
        title="Meetings and discussion groups"
        subtitle="Video meetings, team sessions and discussion groups"
        rows={meetings}
        columns={[
          ["title", "Meeting"],
          ["starts_at", "Starts"],
          ["meeting_provider", "Platform"],
          ["meeting_url", "Join link"],
        ]}
        downloadable
      />
    </>
...
function CalendarPage({
  ...
  const combined: DataRow[] = [
    ...
    ...learning.map((row) => ({
      ...row,
      title: row.course_name ?? "Training",
      event_type: "Training",
  ...
  return (
    <>
      <header className="page-header">
        <div>
          <h1>
            <MenuIcon name={moduleIcon("Help Center")} />
            Help Center
          </h1>
          <p className="muted">
            HR support, IT support, tickets, knowledge and FAQs.
          </p>
        </div>
        <div className="page-actions">
          ...
          <button className="primary" onClick={onTicket}>
            Submit ticket
          </button>
        </div>
      </header>
      <div className="employee-profile-grid">
        <article className="card data-panel">
          <h2>HR Support</h2>
          <p>
            Employment, leave, payroll, benefits, documents and workplace
            questions.
          </p>
        </article>
        <article className="card data-panel">
          <h2>IT Support</h2>
          <p>Login, device, software, access, email and system issues.</p>
        </article>
        <article className="card data-panel">
          <h2>Knowledge base</h2>
          <p>
            Company policies, employee handbook, processes and frequently asked
            questions.
          </p>
        </article>
      </div>
      <RecordPage
        ...
        subtitle="Track responses and resolution"
        rows={tickets}
        columns={[
          ["ticket_type", "Type"],
          ["subject", "Subject"],
...
function NotificationsPage({
  ...
  async function markAll() {
    setBusy(true);
    try {
      await callRpc(accessToken, "mark_all_my_notifications_read", {});
      await onReload();
    } finally {
      setBusy(false);
    }
  ...
  return (
    <RecordPage
      title="Notifications"
      subtitle="Attendance, leave, payroll, performance, training, document and company alerts"
      rows={rows}
      columns={[
        ["title", "Notification"],
      ...
      action={
        <div className="row-actions">
          ...
          <button className="primary" onClick={onPreferences}>
  ...
  return (
    <>
      <header className="page-header">
        <div>
          <h1>
            <MenuIcon name={moduleIcon("Analytics")} />
            My Analytics
          </h1>
          <p className="muted">Private employee summaries only.</p>
        </div>
      </header>
      <div className="employee-quick-cards">
        <article>
        ...
        <article>
          ...
          <strong>{String(performance[0]?.rating ?? "—")}</strong>
        </article>
        <article>
...
function EmployeeSettings({
  ...
  const [language, setLanguage] = useState(
    String(preferences?.language ?? "English"),
  );
  const [theme, setTheme] = useState(String(preferences?.theme ?? "system"));
  const [accessibility, setAccessibility] = useState(
    String(preferences?.accessibility ?? "standard"),
  );
  const [textSize, setTextSize] = useState(
    String(preferences?.text_size ?? "medium"),
  );
  const [density, setDensity] = useState(
    String(preferences?.ui_density ?? "comfortable"),
  );
  const [visibility, setVisibility] = useState(
    String(preferences?.profile_visibility ?? "team"),
  );
  ...
  async function uploadPhoto(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      await updateRow(accessToken, "profiles", profile.id, {
        avatar_path: path,
      });
      setPhotoPath(path);
      setPhotoMessage(
        "Photo updated. It will also appear elsewhere in the app after your next sign-in.",
      );
    } catch (cause) {
      setPhotoMessage(
        cause instanceof Error ? cause.message : "Photo could not be uploaded.",
      );
    } finally {
      setPhotoBusy(false);
    }
  ...
  async function save() {
    ...
    try {
      await callRpc(accessToken, "upsert_my_user_preferences", {
        ...
        p_ui_density: density,
        p_profile_visibility: visibility,
        p_show_email: showEmail,
        p_show_phone: showPhone,
        p_show_birthday: Boolean(preferences?.show_birthday),
        p_show_last_active: preferences?.show_last_active !== false,
        p_allow_location_for_attendance:
          preferences?.allow_location_for_attendance !== false,
        p_allow_ai_personalisation:
          preferences?.allow_ai_personalisation !== false,
      });
  ...
  return (
    <>
      <header className="page-header">
        <div>
          <h1>
            <MenuIcon name={moduleIcon("Settings")} />
            Settings
          </h1>
          <p className="muted">
            Profile, security, notifications, language, appearance,
      ...
      <div className="employee-settings-layout">
          ...
          <article className="card data-panel">
            ...
            <div className="settings-photo-row">
              <AvatarPhoto
                ...
                path={
                  photoPath ||
                  (employee?.passport_photo_path
                    ? String(employee.passport_photo_path)
                    : undefined)
                }
            ...
            <p className="muted settings-note">
              Two-factor authentication is controlled through the secure account
              provider and will appear here when enabled for the organisation.
            </p>
          </article>
          <article className="card data-panel">
            <h2>Notifications</h2>
            <p className="muted">
              Choose email and in-app notification categories from the
              notification preferences panel.
            </p>
            <button className="secondary" onClick={onNotifications}>
              Notification preferences
            </button>
        ...
        <div className="settings-column">
          <article className="card data-panel settings-preferences-card">
            ...
            <label className="check">
              ...
              />{" "}
              Show work phone in directory
            </label>
            {message && (
              <p
                className={
                  message.includes("saved") ? "form-message" : "form-error"
                }
              >
                {message}
              </p>
...
function AiPage({
  ...
  return (
    <section className="ai-assistant-page">
      <header className="page-header">
        <div>
          <span className="eyebrow">BlumeByte Intelligence</span>
          <h1>AI HR Assistant</h1>
          <p className="muted">
            Leave recommendations, career coaching, training suggestions, policy
            search, document help and FAQs.
          </p>
        </div>
      </header>
      <article className="card ai-chat">
        ...
        <form onSubmit={onSubmit}>
          <input
            ...
            placeholder="Ask a question about your employee workspace..."
          />
...
function TaskPage({
  ...
  const [selected, setSelected] = useState<DataRow | null>(null),
    [status, setStatus] = useState("not_started"),
    [progress, setProgress] = useState(0),
    [note, setNote] = useState(""),
    [busy, setBusy] = useState(false),
    [error, setError] = useState("");
  async function open(row: DataRow) {
    setSelected(row);
    setStatus(String(row.status ?? "not_started"));
    setProgress(Number(row.progress ?? 0));
    setNote(String(row.status_note ?? ""));
    if (!row.opened_at)
      try {
        await updateRow(accessToken, "tasks", String(row.id), {
          opened_at: new Date().toISOString(),
          acknowledged_at: new Date().toISOString(),
        });
        await onReload();
      } catch {
        /* Status can still be viewed if acknowledgement logging fails. */
      }
  }
  async function save() {
    ...
    setError("");
    try {
      await updateRow(accessToken, "tasks", String(selected.id), {
        status,
        progress,
        status_note: note,
  ...
  return (
    <article className="card data-panel employee-record-page">
      <div className="panel-head">
        <div>
          <h2>My tasks</h2>
          <p className="muted">
            Open assignments to acknowledge them and report progress to the
            assigner.
          </p>
        </div>
      </div>
      {rows.length ? (
        ...
        <div className="empty-state">
          ...
          <p>New assignments and their descriptions will appear here.</p>
        </div>
      )}
      {selected && (
        <div
        ...
        >
          <section className="modal record-modal">
            ...
            <span className="eyebrow">Task update</span>
            <h2>{String(selected.title)}</h2>
            <p>{String(selected.description ?? "No description provided.")}</p>
            <div className="record-form">
              ...
              <label className="wide">
                ...
                <textarea
                  ...
                  placeholder="Describe progress, blockers, or proof of work."
                />
              </label>
    ...
    <article className="card data-panel employee-record-page">
      ...
      <div className="filter-toolbar">
        ...
        {downloadable && (
          <button
            className="secondary"
            onClick={() => printRows(title, visible, columns)}
          >
            Download / Print PDF
          </button>
        )}
...
function Rows({
  ...
  const withActions = Boolean(accessToken && table && onReload);
  return rows.length ? (
    <div className="table-scroll">
      <table className="data-table">
        <thead>
    ...
    <div className="empty-state">
      ...
      <p>No authorised information has been added to this section.</p>
    </div>
  );
}
function DetailCard({
...
function SelfServiceDialog({
  ...
  type: "leave" | "expense" | "asset" | "ticket" | "profile" | "transfer";
  accessToken: string;
  profile: UserProfile;
  employee: DataRow | null;
  onClose: () => void;
  ...
  const configs = {
    ...
    ticket: {
      ...
      table: "support_tickets",
      fields: [
        ["ticket_type", "Support type", "select"],
    ...
    profile: {
      ...
      fields: [
        ["field_name", "Information to change", "select"],
        ["new_value", "Requested value", "text"],
        ["reason", "Reason", "textarea"],
      ],
    },
    transfer: {
      ...
      table: "transfer_requests",
      fields: [
        ["requested_department", "Requested department", "text"],
        ["requested_branch", "Requested branch", "text"],
        ["reason", "Reason", "textarea"],
      ],
    },
  } as const;
      ...
      <section className="modal record-modal" role="dialog" aria-modal="true">
        ...
        <form className="record-form" onSubmit={submit}>
          {config.fields.map(([key, label, kind]) => (
            <label
              htmlFor={`employee-${type}-${key}`}
              className={kind === "textarea" ? "wide" : ""}
              key={key}
            >
              {label}
              {kind === "textarea" ? (
                <textarea
                  id={`employee-${type}-${key}`}
                  name={key}
                  required
                  value={values[key] ?? ""}
                  onChange={(event) =>
                ...
                >
                  ...
                  {optionsFor(type, key).map((option) => (
                ...
                <input
                  id={`employee-${type}-${key}`}
                  name={key}
                  required={key !== "receipt_url"}
                  min={kind === "number" ? "0.01" : undefined}
...
function optionsFor(type: string, key: string) {
  if (type === "leave")
    return [
      "Annual leave",
      "Sick leave",
  ...
  if (key === "priority") return ["low", "normal", "high", "urgent"];
  if (key === "ticket_type")
    return [
      "HR Support",
      "IT Support",
