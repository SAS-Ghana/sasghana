...
import { printTemplateDocument, renderTemplateContent } from "./lib/document-templates";
...
const empty: Dataset = {
  ...
  notifications: [], requests: [], awards: [], history: [], preferences: [], transfers: [],
};

export function EmployeeHome({ accessToken, profile, activeSection = "Home", onNavigate, onChangePassword, onNotificationSettings, onLogout }: EmployeeHomeProps) {
  ...
  const load = useCallback(async () => {
    ...
    const [employee, attendance, leave, payroll, documents, performance, goals, learning, tasks, expenses, assets, assetRequests, benefits, jobs, applications, announcements, messages, meetings, holidays, tickets, notifications, requests, awards, history, preferences, transfers] = await Promise.all([
    ...
    setData({ employee, attendance, leave, payroll, documents, performance, goals, learning, tasks, expenses, assets, assetRequests, benefits, jobs, applications, announcements, messages, meetings, holidays, tickets, notifications, requests, awards, history, preferences, transfers });
  ...
  return <section className="employee-portal-v2">
    ...
    {activeSection === "Home" && <div className="home-hero-row employee-home-hero">
      ...
      <article className="card employee-clock quick-attendance-card compact"><div className="panel-head"><div><span className="eyebrow">Live attendance</span><h2>{onBreak ? "On break" : clockedIn ? "Working" : "Ready to start"}</h2><p className="muted">{onBreak ? "You are currently on break." : clockedIn ? "Your workday is active." : "Clock in to begin today's attendance."}</p></div><strong className="attendance-live-time">{clockedIn ? liveTime : "00:00"}</strong></div><div className="row-actions attendance-quick-actions"><button className="primary" disabled={clockedIn || Boolean(busy)} onClick={() => void attendanceAction("clock_in")}>Clock in</button><button disabled={!clockedIn || onBreak || Boolean(busy)} onClick={() => void attendanceAction("break_in")}>Start break</button><button disabled={!onBreak || Boolean(busy)} onClick={() => void attendanceAction("break_out")}>Resume</button><button className="danger" disabled={!clockedIn || Boolean(busy)} onClick={() => void attendanceAction("clock_out")}>Clock out</button></div></article>
    </div>}
    ...
    {tab === "help" && <HelpPage tickets={data.tickets} onTicket={() => setModal("ticket")} onAi={() => setTab("ai")} accessToken={accessToken} onReload={load} />}
...
function DashboardOverview({ employee, data, metrics, clockedIn, onTab, onProfile, onAskAi }: { employee: DataRow | null; data: Dataset; metrics: { leaveBalance: number; pending: number; attendanceRate: number; overtime: number; openTasks: number; unread: number; performanceScore: number }; clockedIn: boolean; onTab: (tab: Tab) => void; onProfile: () => void; onAskAi: () => void }) {
  ...
  return <>
    ...
    <div className="employee-dashboard-grid">
      <article className="card employee-profile-card"><h2>Employee profile</h2><div className="profile-summary"><div className="profile-photo">{String(employee?.first_name ?? "E").slice(0, 1)}{String(employee?.last_name ?? "").slice(0, 1)}</div><div><h3>{`${employee?.first_name ?? ""} ${employee?.last_name ?? ""}`.trim() || "Employee"}</h3><p>{String(employee?.position_title ?? "Employee")}</p><span>{String(employee?.employee_number ?? "Staff number pending")}</span></div></div><button className="secondary wide-button" onClick={onProfile}>View full profile</button></article>
      ...
      ].sort((a, b) => String(b.created_at ?? "").localeCompare(String(a.created_at ?? ""))).slice(0, 5).map((row) => ({ icon: "profile-request" as IconName, iconColor: "var(--sas-cyan-deep)", title: String(row.request_label), subtitle: String(row.status ?? "pending").replaceAll("_", " ") }))} emptyLabel="No requests submitted yet." />
...
function PerformancePage({ reviews, goals, awards }: { reviews: DataRow[]; goals: DataRow[]; awards: DataRow[] }) { return <><div className="employee-quick-cards"><article><span>Performance score</span><strong>{String(reviews[0]?.rating ?? reviews[0]?.score ?? "—")}</strong></article><article><span>Active goals</span><strong>{goals.filter((row) => row.status !== "completed").length}</strong></article><article><span>Completed goals</span><strong>{goals.filter((row) => row.status === "completed").length}</strong></article><article><span>Awards</span><strong>{awards.length}</strong></article></div><RecordPage title="Performance reviews" subtitle="Self assessment, manager feedback and review history" rows={reviews} columns={[["review_type", "Review"], ["review_period", "Period"], ["rating", "Score"], ["self_assessment", "Self assessment"], ["manager_feedback", "Manager feedback"], ["status", "Status"]]} downloadable /><RecordPage title="Goals and objectives" subtitle="KPI and OKR progress" rows={goals} columns={[["title", "Goal"], ["target_value", "Target"], ["progress", "Progress"], ["due_date", "Due"], ["status", "Status"]]} downloadable /><RecordPage title="Recognition and awards" subtitle="Recognition, awards and career development" rows={awards} columns={[["title", "Award"], ["award_date", "Date"], ["description", "Details"]]} downloadable /></>; }
function LearningPage({ rows }: { rows: DataRow[] }) { return <RecordPage title="Learning & Development" subtitle="Assigned courses, company training, certifications, skills and workshops" rows={rows} columns={[["course_name", "Course"], ["training_type", "Type"], ["progress", "Progress"], ["due_date", "Due"], ["certificate_status", "Certificate"], ["status", "Status"]]} downloadable />; }
function BenefitsPage({ rows }: { rows: DataRow[] }) { return <RecordPage title="Benefits" subtitle="Health insurance, pension, medical benefits, staff loans, allowances and enrollment" rows={rows} columns={[["benefit_name", "Benefit"], ["plan_name", "Plan"], ["provider", "Provider"], ["start_date", "Starts"], ["end_date", "Ends"], ["enrollment_status", "Enrollment"], ["status", "Status"]]} downloadable />; }
...
function RecruitmentPage({ jobs, applications, transfers, employeeId, organisationId, accessToken, onTransfer, onReload }: { jobs: DataRow[]; applications: DataRow[]; transfers: DataRow[]; employeeId?: string; organisationId: string; accessToken: string; onTransfer: () => void; onReload: () => Promise<void> }) {
  ...
  return <>
    ...
    <div className="job-cards">{openJobs.map((job) => <article className="card job-card" key={String(job.id)} onClick={() => setViewing(job)} role="button" tabIndex={0} onKeyDown={(event) => { if (event.key === "Enter") setViewing(job); }}>
      <span className="eyebrow">{String(job.employment_type ?? "Internal opportunity")}</span>
      <h2>{String(job.title)}</h2>
      <p>{String(job.location ?? "")} · closes {value(job.closing_date)}</p>
      <p className="job-card-excerpt">{String(job.description ?? "").slice(0, 140)}{String(job.description ?? "").length > 140 ? "…" : ""}</p>
      <div className="row-actions"><button type="button" className="secondary" onClick={(event) => { event.stopPropagation(); setViewing(job); }}>View details</button><button type="button" className="primary" onClick={(event) => { event.stopPropagation(); setApplying(job); }}>Apply internally</button></div>
    </article>)}{!openJobs.length && <div className="empty-state"><h3>No open vacancies</h3><p>Internal opportunities will appear here when published.</p></div>}</div>
    <RecordPage title="Application tracking" subtitle="Your internal applications and status" rows={trackedApplications} columns={[["job_title", "Role"], ["created_at", "Applied"], ["status", "Status"], ["cover_note", "Cover note"]]} downloadable />
    <RecordPage title="My transfer requests" subtitle="Requested department, branch and manager decisions" rows={transfers} columns={[["requested_department", "Department"], ["requested_branch", "Branch"], ["status", "Status"], ["manager_comment", "Manager update"], ["created_at", "Submitted"]]} downloadable accessToken={accessToken} table="transfer_requests" onReload={onReload} />
    {viewing && <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setViewing(null); }}><section className="modal" role="dialog" aria-modal="true"><button type="button" className="modal-close" onClick={() => setViewing(null)}>×</button><span className="eyebrow">{String(viewing.employment_type ?? "Internal opportunity")}</span><h2>{String(viewing.title)}</h2><p className="muted">{String(viewing.location ?? "")} · closes {value(viewing.closing_date)}</p><h3>Description</h3><p style={{ whiteSpace: "pre-wrap" }}>{String(viewing.description ?? "No description provided.")}</p>{viewing.requirements ? <><h3>Requirements</h3><p style={{ whiteSpace: "pre-wrap" }}>{String(viewing.requirements)}</p></> : null}<div className="form-actions"><button type="button" className="secondary" onClick={() => setViewing(null)}>Close</button><button type="button" className="primary" onClick={() => { setApplying(viewing); setViewing(null); }}>Apply internally</button></div></section></div>}
    {applying && <ApplyDialog job={applying} employeeId={employeeId} organisationId={organisationId} accessToken={accessToken} onClose={() => setApplying(null)} onSaved={async () => { setApplying(null); await onReload(); }} />}
  </>;
}

function ApplyDialog({ job, employeeId, organisationId, accessToken, onClose, onSaved }: { job: DataRow; employeeId?: string; organisationId: string; accessToken: string; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  async function submit(event: FormEvent) {
    event.preventDefault();
    if (!employeeId) return setError("Your login is not linked to an employee record.");
    ...
    try {
      ...
      if (resume) {
        ...
        await uploadStorageFile(accessToken, "employee-media", resumePath, resume);
      }
      await createRow(accessToken, "internal_job_applications", {
        organisation_id: organisationId, job_opening_id: job.id, employee_id: employeeId,
        status: "submitted", cover_note: coverNote.trim() || null, resume_path: resumePath || null,
      });
      await onSaved();
...
function RecordPage({ title, subtitle, rows, columns, action, downloadable = false, accessToken, table, onReload }: { title: string; subtitle: string; rows: DataRow[]; columns: [string, string][]; action?: ReactNode; downloadable?: boolean; accessToken?: string; table?: string; onReload?: () => Promise<void> }) {
  ...
  return <article className="card data-panel employee-record-page"><div className="panel-head"><div><h2>{title}</h2><p className="muted">{subtitle}</p></div>{action}</div><div className="filter-toolbar"><input aria-label={`Search ${title}`} value={query} onChange={(event) => setQuery(event.target.value)} placeholder={`Search ${title.toLowerCase()}...`} /><select aria-label={`Sort ${title}`} value={sort} onChange={(event) => setSort(event.target.value)}>{columns.map(([key, label]) => <option key={key} value={key}>Sort by {label}</option>)}</select><select aria-label="Sort direction" value={direction} onChange={(event) => setDirection(event.target.value as "asc" | "desc")}><option value="desc">Newest first</option><option value="asc">Oldest first</option></select>{downloadable && <button className="secondary" onClick={() => printRows(title, visible, columns)}>Download / Print PDF</button>}</div><Rows rows={visible} columns={columns} accessToken={accessToken} table={table} onReload={onReload} /></article>;
}
function Rows({ rows, columns, accessToken, table, onReload }: { rows: DataRow[]; columns: [string, string][]; accessToken?: string; table?: string; onReload?: () => Promise<void> }) {
  const withActions = Boolean(accessToken && table && onReload);
  return rows.length ? <div className="table-scroll"><table className="data-table"><thead><tr>{columns.map(([, label]) => <th key={label}>{label}</th>)}{withActions && <th>Actions</th>}</tr></thead><tbody>{rows.map((row, index) => <tr key={String(row.id ?? index)}>{columns.map(([key]) => <td key={key}>{value(row[key])}</td>)}{withActions && <td><div className="row-actions"><SelfRecordActions accessToken={accessToken!} table={table!} row={row} columns={columns} onReload={onReload!} /></div></td>}</tr>)}</tbody></table></div> : <div className="empty-state"><h3>No records yet</h3><p>No authorised information has been added to this section.</p></div>;
}
