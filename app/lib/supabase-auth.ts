...
export async function fetchProfile(accessToken:string,userId:string){const [profileResponse,permissionsResponse,rolesResponse]=await Promise.all([
  fetch(`${serviceUrl}/rest/v1/profiles?select=id,organisation_id,username,email,display_name,status,account_type,job_title,employee_id,dashboard_access,preferred_dashboard,self_service_enabled,two_step_email_enabled,two_step_email_verified_at&id=eq.${userId}`,{headers:{apikey:publishableKey,Authorization:`Bearer ${accessToken}`}}),
...
export function saveSession(session:AuthSession,remember:boolean){(remember?localStorage:sessionStorage).setItem("sas-people-session",JSON.stringify(normaliseSession(session)));}
