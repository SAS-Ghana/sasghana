...
const dashboardAccessMap: Record<string, readonly string[]> = {
  ...
  "My Team": ["Directory", "Employees"],
  Recruitment: ["Hiring"],
  "Recruitment & Onboarding": ["Hiring", "Onboarding"],
  ...
  Assets: ["Assets"],
  "Announcements & Communication": ["Announcements", "Community"],
...
export function PeopleDashboard({
  ...
  const libraryGranted =
    mode !== "admin" &&
    Boolean(profile.dashboard_access?.includes("Book Library"));
  ...
  const documentStudioGranted =
    mode !== "admin" &&
    mode !== "hr" &&
    profile.permissions.includes("documents.verify");
  ...
  const liveAttendanceGranted =
    mode === "manager" &&
    Boolean(profile.dashboard_access?.includes("Live Attendance"));
  const extraGroups: (readonly [
    string,
    readonly (readonly [string, string])[],
  ])[] = [];
  if (libraryGranted && mode !== "employee")
    extraGroups.push(["LIBRARY", [["Book Library", "📚"]]] as const);
  if (documentStudioGranted && mode === "manager")
    extraGroups.push(["DOCUMENTS", [["Documents & Templates", "◫"]]] as const);
  if (liveAttendanceGranted)
    extraGroups.push(["ATTENDANCE", [["Live Attendance", "◷"]]] as const);
  if (canProcurement && mode !== "admin" && mode !== "employee")
    extraGroups.push(["PROCUREMENT", [["Procurement Review", "▣"]]] as const);
  ...
  const canAccess = (label: string) =>
    ...
    (labels.has(label) ||
      (label === "Book Library" && libraryGranted) ||
      (label === "Documents & Templates" && documentStudioGranted) ||
      (label === "Live Attendance" && liveAttendanceGranted) ||
      (label === "Procurement Review" && canProcurement)) &&
    (label === home || hasDashboardAccess(mode, label, granted));
  ...
  useEffect(() => {
    const loadBranding = () =>
      void loadOrganisationConfig(accessToken, profile.organisation_id).then(
        (next) => {
          setBranding(next);
    ...
    const channel = client
      .channel(`branding-${profile.organisation_id}`)
      .on(
        "postgres_changes",
        {
          event: "*",
          schema: "public",
          table: "public_branding",
      ...
      .subscribe();
    return () => {
      window.removeEventListener("sas-branding-changed", loadBranding);
  ...
  const render = () => {
    ...
    if (active === "Profile Requests")
      return <ProfileRequestsPage accessToken={accessToken} />;
    ...
    if (active === "Documents & Templates" && documentStudioGranted)
      return (
        <DocumentStudio
          accessToken={accessToken}
          organisationId={profile.organisation_id}
    ...
    if (mode === "admin") {
      if (active === "Administrator Dashboard")
        return (
          <AdminDashboard
            accessToken={accessToken}
            profile={profile}
      ...
      if (active === "User & Account Management")
        return <AccountManagementPage accessToken={accessToken} />;
      if (active === "Employee Management")
        return (
          <PeopleDirectory
            accessToken={accessToken}
            canManage
      ...
      if (active === "Organization Structure")
        return <DepartmentHub accessToken={accessToken} profile={profile} />;
      if (active === "Attendance Management")
        return <AttendanceHub accessToken={accessToken} />;
      if (active === "Live Attendance")
        return (
          <LiveAttendancePage accessToken={accessToken} profile={profile} />
        );
      if (active === "Onboarding")
        return <OnboardingHub accessToken={accessToken} profile={profile} />;
      if (active === "Performance Management")
    ...
    if (mode === "hr") {
      ...
      if (active === "Payroll Administration")
        return (
          <PayrollSettingsPage
            accessToken={accessToken}
            organisationId={profile.organisation_id}
  ...
  return (
    <div className={`app ${mode === "employee" ? "employee-app" : ""}`}>
      <div
      ...
      >
        <div className="brand">
          <img
            src={branding.dashboardLogoUrl || branding.logoUrl || "/logo.png"}
            width="160"
            height="45"
        ...
        <header className="topbar">
          ...
          <div className="search-wrap">
            ...
            <input
              ...
              placeholder={
                mode === "admin"
                  ? "Search organization administration..."
                  : mode === "hr"
                    ? "Search HR workspace..."
                    : mode === "employee"
                      ? "Search..."
          ...
          <div
            className="profile"
            ref={mode === "employee" ? undefined : accountRef}
          >
            <NotificationCenter
              accessToken={accessToken}
              profile={profile}
            ...
            {mode !== "employee" && (
              ...
              >
                ...
                <div className="profile-copy">
                  ...
                  <small>{profile.roles[0] ?? profile.account_type}</small>
                </div>
              </button>
            )}
            {mode !== "employee" && accountOpen && (
              <div className="account-menu">
                <button onClick={() => navigate("My Profile")}>
                  My profile
                </button>
                <button onClick={() => setNotificationSettings(true)}>
