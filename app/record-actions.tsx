...
export function RecordActions({
  ...
  async function remove() {
    ...
    if (
      !window.confirm(
        `Delete this ${label.toLowerCase()} record? This cannot be undone.`,
      )
    )
      return;
    setBusy(true);
    ...
    try {
      ...
      setError(
        cause instanceof Error ? cause.message : "Record could not be deleted.",
      );
    } finally {
      setBusy(false);
    }
...
export function SelfRecordActions({
  accessToken,
  table,
  row,
  columns,
  ...
  async function withdraw() {
    if (!row.id) return;
    if (!window.confirm("Withdraw this request? This cannot be undone."))
      return;
    setBusy(true);
    setError("");
    try {
      await updateRow(accessToken, table, String(row.id), {
        status: "cancelled",
      });
      await onReload();
    } catch (cause) {
...
function GenericEditDialog({
  ...
  const needsEmployeeOptions = columns.some(([key]) =>
    ["employee_id", "assigned_employee_id"].includes(key),
  );
  ...
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  const [employeeOptions, setEmployeeOptions] = useState<DataRow[]>([]);

  useEffect(() => {
    if (!needsEmployeeOptions) return;
    void listNamedRows(
      accessToken,
      "employees",
  ...
  }, [accessToken, needsEmployeeOptions]);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      for (const key of [...derivedFields, ...protectedFields])
        delete payload[key];
  ...
  return (
    ...
    >
      <section
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="generic-edit-title"
      >
        <button
          type="button"
        ...
        <form className="record-form" onSubmit={submit}>
          {columns.map(([key, fieldLabel]) => {
            ...
            if (["employee_id", "assigned_employee_id"].includes(key))
              return (
                <label key={key}>
                  {fieldLabel}
                  <select
